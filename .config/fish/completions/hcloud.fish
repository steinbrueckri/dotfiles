# fish completion for hcloud                               -*- shell-script -*-
...
function __hcloud_perform_completion
    __hcloud_debug "Starting __hcloud_perform_completion"
    ...
    __hcloud_debug "args: $args"
    __hcloud_debug "last arg: $lastArg"

    # Disable ActiveHelp which is not supported for fish shell
    set -l requestComp "HCLOUD_ACTIVE_HELP=0 $args[1] __complete $args[2..-1] $lastArg"
    ...
    __hcloud_debug "Comps: $comps"
    __hcloud_debug "DirectiveLine: $directiveLine"
...
function __hcloud_prepare_completions
    __hcloud_debug ""
    ...
    if test $compErr -eq 1
        __hcloud_debug "Received error directive: aborting."
        # Might as well do file completion, in case it helps
        return 1
    end
    ...
    if test $filefilter -eq 1; or test $dirfilter -eq 1
        __hcloud_debug "File extension filtering or directory filtering not supported"
    ...
    __hcloud_debug "nospace: $nospace, nofiles: $nofiles"
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __hcloud_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__hcloud_comp_results)
        set --global __hcloud_comp_results $completions
        __hcloud_debug "Filtered completions are: $__hcloud_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__hcloud_comp_results)
        __hcloud_debug "numComps: $numComps"
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__hcloud_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                __hcloud_debug "Adding second completion to perform nospace directive"
                set --global __hcloud_comp_results $split[1] $split[1].
                __hcloud_debug "Completions are now: $__hcloud_comp_results"
            end
        end
        ...
        if test $numComps -eq 0; and test $nofiles -eq 0
            ...
            __hcloud_debug "Requesting file completion"
            return 1
        end
    end
...
if type -q "hcloud"
    ...
    complete --do-complete "hcloud " > /dev/null 2>&1
end
